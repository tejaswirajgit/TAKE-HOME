...
export function CompletionScreen({ intake }: { intake: Intake }) {
  ...
  const sampleLabel = sampleOpt ? (lang === "en" ? optLabel(sampleOpt, lang).toLowerCase() : optLabel(sampleOpt, lang)) : "";
  const consented = answers.consent === "yes";
  const nextLine = consented
    ? sampleLabel && answers.sample_type !== "either"
      ? s.doneSample(sampleLabel)
      : ""
    : s.doneNoSample;
  ...
  useEffect(() => {
    h1.current?.focus();
    if (readAloud) speech.speak(`${s.doneTitle} ${s.doneBody} ${nextLine}`, lang);
    return () => speech.stop();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [readAloud, lang]);

  return (
    <main className="frame flex min-h-dvh flex-col px-6 py-5">
      <TopBar lang={lang} setLang={intake.setLang} readAloud={readAloud} setReadAloud={intake.setReadAloud} />
