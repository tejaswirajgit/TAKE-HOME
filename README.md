...
- **Voice**: for someone who is blind, low-vision, or simply tired of tapping. Turning it on plays one short intro ("I will read each question. After the beep, say your answer, or its number…"); then each question is read in the chosen language (Sarvam `bulbul:v3`, one pinned voice), a beep sounds, the mic opens by itself and closes after ~1.2 s of silence. Options are read only on a miss or on "Read this again", so a question takes seconds, not a paragraph; the app reads back what it understood ("Father, Siblings — is that right?"); **"haan" / "yes" confirms, "nahi" redoes**, anything else is taken as a new answer. Answers can be the option itself in English, Hinglish or Hindi ("papa aur bhai", "lagu nahi hota"), or its number as read out ("option 2", "teesra", "last"). Something that is not an option ("uncle") gets the options read back with their numbers rather than a bare "didn't catch that"; "pata nahi" is never turned into a "no". The question and its chips stay on screen the whole time, so a caregiver can follow along and any tap still wins. After two misses the mic stops re-opening on its own — the pill and the chips are still there.
...
**Every option, on the live site (31 Aug, after deploy):** a headless-browser script drove the deployed URL through eight complete intakes that between them tap *every option of every question* — each habits row both ways with every follow-up, every product and procedure row with every column value, Q14 with and without a description, consent yes and no — alternating 390 px and 1440 px, once in Hindi, once driven only by the keyboard (1–9, Enter), plus a reload-and-resume mid-way and an Edit-from-review. After each intake the review-screen JSON was compared with the expected official values and validated against `intake-schema.json`: 24/24, zero console errors. The same was done for voice: 56 Hindi phrases covering every option were synthesized with Sarvam TTS, sent through the live `/api/stt`, and the transcripts run through the rules — 53 understood end to end; the three misses were synthetic-speech artefacts ("रेगुलर" came back as "Ragnar"). That run also showed Sarvam romanizes "कुछ" as *kuchh* and "माथे" as *mathhe*, so the parser now folds doubled-h spellings on both sides of every match.

**Noise (a waiting room is never quiet):** the Hindi clip was mixed with steady noise and with someone else talking, then run through the live STT and through the hands-free loop with the noisy file as the microphone. Steady noise — fans, traffic, an AC — is a non-issue for Sarvam: the transcript was character-perfect even at 0 dB (noise as loud as the voice). That run exposed a real bug in the silence detector: the room noise itself counted as "speech", so the mic closed before the patient began. It now calibrates on the first 300 ms, treats speech as anything clearing 2.5× the room for two frames in a row, and asks the browser for noise suppression and gain control — at 10 dB the mic waits through the noise, captures the answer and closes after it; at 0 dB hands-free gives up after 5 s and the patient taps or holds the pill instead. Competing speech is the honest limit: a neighbour talking at equal loudness straight into the mic gets transcribed too — which is exactly why nothing is committed until the read-back is confirmed.
